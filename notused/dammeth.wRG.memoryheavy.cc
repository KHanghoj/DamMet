  ...
  for (size_t i = 0; i < d.t_seq.size()-1; i++){
	...
	// continue;
      ...
      if(no_ref_cpg(r_base1, r_base2)){
        if(refToInt[(int)ref[d.t_positions[b2]]]==2 &&
           nocpg_data.depth < nocpg_data.max_depth){
	...
	// continue;
      ...
      if(no_ref_cpg(r_base1, r_base2)){
        if(refToInt[(int)ref[d.t_positions[b1]]]==1 && nocpg_data.depth < nocpg_data.max_depth){
          nocpg_data.pos_to_end.push_back(pos);
          nocpg_data.prime.push_back(prime);
          nocpg_data.base_compos.push_back(base_composition);
          nocpg_data.seqerrors.push_back(seqerror);
...
struct F_void {
  ...
  std::vector<pre_calc_per_site2> * data;
  per_mle_run2 * mle_data;
  size_t iteration;
  F_void(const general_settings & s, std::vector<pre_calc_per_site2> * d, per_mle_run2 * m){
    settings = s;
    data = d;
    mle_data = m;
    iteration = 0;
...
void run_deamrates_optim(const general_settings & settings, const tally_mat_freq & tmf, std::vector<per_site> & data, per_site_nocpg & nocpg_data , std::string & rg){
...
void update_mle_data(per_mle_run2 & mle_data, const pre_calc_per_site2 & data, size_t & idx){
...
double objective_func_F_second_deriv(const double & f, std::vector<pre_calc_per_site2> & data, per_mle_run2 &mle_data){
  double slope=0;
...
void run_mle(const general_settings & settings,
             std::vector<pre_calc_per_site2> & pre_calc_data) {
  ...
  for (size_t curr_idx = 0; curr_idx < pre_calc_data.size(); curr_idx++) {
    per_mle_run2 mle_data(pre_calc_data[curr_idx], curr_idx);
...
int parse_bam(int argc, char * argv[]) {
  ...
  if(!settings.deamrates_f_rg.empty() && check_file_exists(settings.deamrates_f_rg)){
    ...
    rg_split = false;
    std::cerr << "\t-> Merging all reads into a single read group named: " << ALL_RG  << '\n';
  ...
  std::vector<std::vector<per_site>> data;
  data.resize(rgs.size());
  for (size_t i=0; i<rgs.size(); i++){
    data[i].resize(cpg_map.size());
  }

  size_t nocpg_to_include = 1e6;
  std::cerr << "\t-> Loading " << nocpg_to_include << " of C's outside a CpG context" << '\n';

  std::vector<per_site_nocpg> nocpg_data; 
  // https://stackoverflow.com/a/7413968/2788987
  nocpg_data.resize(rgs.size(), per_site_nocpg(nocpg_to_include));
  ...
  while (sam_itr_next(in, iter, rd) >= 0) {
   ...
   d = align_read(rd, ref);
   add_aligned_data(settings, d, cpg_map, ref, data[rgname_idx], tm[rgname_idx], nocpg_data[rgname_idx]);
  }
  ...
  for (size_t i=0; i<rgs.size(); i++){
    ...
    std::cerr << "\t-> Total_Observations RG: "<< rgs[i] << " CpG: " << cpgsdepth  << ". CnoCpG: " << nocpg_data[i].depth << '\n';
  }
  ...
  for (size_t i=0; i<rgs.size(); i++){
    ...
    if(!settings.deamrates_filename.empty() && check_file_exists(settings.deamrates_filename)){
      ...
      std::cerr << "\t-> Starting Optim of deamination rates." << '\n';
      run_deamrates_optim(settings, tmf[i], data[i], nocpg_data[i], rgs[i]);
      std::cerr << "\t-> Dumping deamination parameters to " << settings.outbase+"."+rgs[i]+".deamrates" << '\n';
      param_deam[i] = load_deamrates(settings, rgs[i]);
    }
  }
  std::vector<pre_calc_per_site2> mle_data;
  mle_data.reserve(cpg_map.size());
  double deamin_methylated, deamin_unmethylated;
  double noM, M;
  std::cerr << "\t-> Precalculating many things for ML of F." << '\n';
  per_site * site;
  for (size_t x=0; x<cpg_map.size(); x++) {
    size_t temp_depth = 0, temp_position=0;
    for (size_t i=0; i<rgs.size(); i++){
      temp_depth += data[i][x].depth;
      if(data[i][x].depth){
	temp_position = data[i][x].position;
      }
    }
    if(!temp_depth){
    ...
    pre_calc_per_site2 d(temp_depth, temp_position);
    for (size_t i=0; i<rgs.size(); i++){
      site = &data[i][x];      
      for (size_t j = 0; j < site->depth; j++) {
	deamin_methylated = param_deam[i][get_param_idx(settings.max_pos_to_end, METHSTATE, site->pos_to_end[j], site->prime[j])];
	deamin_unmethylated = param_deam[i][get_param_idx(settings.max_pos_to_end, UNMETHSTATE, site->pos_to_end[j], site->prime[j])];
	noM = calc_prob_obs_base(site->seqerrors[j], site->base_compos[j], deamin_unmethylated);
	M = calc_prob_obs_base(site->seqerrors[j], site->base_compos[j], deamin_methylated);

	d.pre_noM.push_back(noM);
	d.pre_M.push_back(M);

	d.maperrors.push_back(site->maperrors[j]);

	d.prob_no_deamin_event_meth *= 1-deamin_methylated;
	d.prob_no_deamin_event_unmeth *= 1-deamin_unmethylated;
	d.summary[site->base_compos[j]]++;
      }
    }
