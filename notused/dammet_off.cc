...
mcmc_meth_mat init_mcmc_meth_mat(const general_settings & settings, const tally_mat_freq & tmf){
  ...
  // std::normal_distribution<double> dist_normal (startval, settings.std_deviation);
  // std::cerr << "init set to zero" << '\n';
...
void get_new_random_values(const general_settings &settings,
  ...
  for (size_t i=0; i<2;i++){  // 0 is meth; 1 is unmeth.
    for (size_t p=0; p<(settings.max_pos_to_end+2);p++){
      for (size_t pr=0; pr<PRIMES; pr++){
        if((pr==1 && p==settings.max_pos_to_end+1) || (pr==1 && p==0)){
          // we have the center pos as 5 prime 20
  ...
  if (f.is_open()) {
    for (const auto &site : data) {
      ...
      f << site.position << " " << site.depth << " " << t->first.first
        ...
        << dinucl_max->second.first << dinucl_max->second.second << " :::";

      for (size_t i = 0; i < site.depth; i++) { // bases
        f << " " << site.bases[i].first << site.bases[i].second;
      }
      // f << " :::";
      // for (size_t i = 0; i < site.depth; i++) { // ref
      //   f << " " << site.refs[i].first << site.refs[i].second;
      // }
      f << " :::";
      for (size_t i = 0; i < site.depth; i++) { // strand
        f << " " << site.strand[i];
      }
      f << " :::";
      for (size_t i = 0; i < site.depth; i++) { // pos to closest termini
        f << " " << site.pos_to_end[i];
      }

      f << " :::";
      for (size_t i = 0; i < site.depth; i++) { // prime
        f << " " << site.prime[i];
      }
      f << " :::";
      for (size_t i = 0; i < site.depth; i++) { // prob of the obs base given CG
        f << " "
          << tmf[site.pos_to_end[i]][site.prime[i]][site.strand[i]][1][2]
                [site.bases[i].first][site.bases[i].second];
      }
      f << '\n';
    }
  }
...
alignment_data align_read(bam1_t * rd, char * ref){
  // following should not be constants:
  alignment_data d ;
...
void tally_aligned_data(const general_settings & settings, const alignment_data & d, tally_mat & tm, const char * ref_tally){
...
void add_aligned_data(const general_settings &settings, const alignment_data &d,
  ...
  // double baseqdeamin = 0;
  // double baseqnodeamin = 0;
  ...
  for (size_t i = 0; i < d.t_seq.size()-1; i++){
    if(d.strand){ // negative strand
      ...
      if(nucleotide_missing(r_base1, r_base2, s_base1, s_base2) ||
         d.t_qs[b1] < settings.minbaseQ ||
         d.t_qs[b2] < settings.minbaseQ ){
        continue;
      }
    ...
    // this is actually not used for anything else than printing.
...
long double mcmc_loglike(const general_settings & settings, const mcmc_meth_mat & d, const std::vector<per_site> & data, const per_site_nocpg & nocpg_data){
  long double ll=0;
  double res=0;
  ...
  for (const auto &site : data) {
    for (size_t i=0; i < site.depth; i++){
      ...
      res = (1-maperror) * (settings.M * M + (1-settings.M)*noM)  + maperror * BASE_FREQ_FLAT_PRIOR;
      ll += std::log(res);
    }
  }

// #ifdef DEBUG_INCL_NOCPG
//   for(size_t i=0; i<nocpg_data.depth; i++){
//     res = 0;
//     seqerror = nocpg_data.seqerrors[i];
//     maperror = nocpg_data.maperrors[i];
//     // deamin_methylated = d[0][nocpg_data.pos_to_end[i]][nocpg_data.prime[i]];
//     deamin_unmethylated = d[1][nocpg_data.pos_to_end[i]][nocpg_data.prime[i]];

//     if (nocpg_data.base_compos[i] == 0) {
//       // C->C -> (1-e) * (1-d) + (d * e/3)
//       noM = (1 - seqerror) * (1 - deamin_unmethylated) + (deamin_unmethylated * seqerror / 3.0);
//     } else if (nocpg_data.base_compos[i] == 1) {
//       // C->T -> (1-e) * d + ((1-d) * e/3)
//       noM = (1 - seqerror) * deamin_unmethylated       + ((1-deamin_unmethylated) * seqerror / 3.0);
//     } else {
//       // C->[AG] -> (d)*e/3.0 + (1-d) * e/3.0
...
void run_mcmc(const general_settings & settings, const tally_mat_freq & tmf, std::vector<per_site> data, const per_site_nocpg & nocpg_data ){
  std::normal_distribution<double> dist_normal (0, settings.std_deviation);
  mcmc_meth_mat d_old = init_mcmc_meth_mat(settings, tmf);
  mcmc_meth_mat d_new = init_mcmc_meth_mat(settings, tmf);
  long double ll_old = 0, ll_new=0, alpha=0;
  ll_old = mcmc_loglike(settings, d_old, data, nocpg_data);
  ...
  // std::cerr << nocpg_data.depth << '\n';

  // std::cerr << "for simple sim" << '\n';
  // std::cerr << "CHANING METHYLATED:" << '\n';
  // d_old[0][10][0] = 0;
  // std::cerr << mcmc_loglike(settings, d_old, data, nocpg_data) << " " << 0 << '\n';
  // d_old[0][10][0] = 0.1;
  // std::cerr << mcmc_loglike(settings, d_old, data, nocpg_data) << " " << 0.1 << '\n';
  // d_old[0][10][0] = 0.2;
  // std::cerr << mcmc_loglike(settings, d_old, data, nocpg_data) << " " << 0.2 << '\n';
  // d_old[0][10][0] = 0.3;
  // std::cerr << mcmc_loglike(settings, d_old, data, nocpg_data) << " " << 0.3 << '\n';
  // d_old[0][10][0] = 0.4;
  // std::cerr << mcmc_loglike(settings, d_old, data, nocpg_data) << " " << 0.4 << '\n';
  // d_old[0][10][0] = 0.5;
  // std::cerr << mcmc_loglike(settings, d_old, data, nocpg_data) << " " << 0.5 << '\n';
  // d_old[0][10][0] = 0.6;
  // std::cerr << mcmc_loglike(settings, d_old, data, nocpg_data) << " " << 0.6 << '\n';
  // d_old[0][10][0] = 0.7;
  // std::cerr << mcmc_loglike(settings, d_old, data, nocpg_data) << " " << 0.7 << '\n';
  // d_old[0][10][0] = 0.8;
  // std::cerr << mcmc_loglike(settings, d_old, data, nocpg_data) << " " << 0.8 << '\n';
  // d_old[0][10][0] = 0.9;
  // std::cerr << mcmc_loglike(settings, d_old, data, nocpg_data) << " " << 0.9 << '\n';
  // d_old[0][10][0] = 1;
  // std::cerr << mcmc_loglike(settings, d_old, data, nocpg_data) << " " << 1 << '\n';
  // std::cerr << "CHANGING UNMETHYLATED:" << '\n';
  // d_old[1][10][0] = 0.1 ;
  // std::cerr << mcmc_loglike(settings, d_old, data, nocpg_data) << " " << 0.1 << '\n';
  // d_old[1][10][0] = 1 ;
  // std::cerr << mcmc_loglike(settings, d_old, data, nocpg_data) << " " << 1 << '\n';
  ...
  while (curr_chain < Nchains){
    ...
    ll_new = mcmc_loglike(settings, d_new, data, nocpg_data);
...
int parse_bam(int argc, char * argv[]) {
  ...
  if(false){
    std::cerr << "\t-> TEMPORARILY:  REMOVING CPGs from REF" << '\n';
    std::string fn ("/Users/krishang/Documents/My_PAPERS/MethylationModel/code_mcmc_mle/temp/meth_low_data_to_be_removed.txt");
  ...
  if(check_file_exists(settings.outbase+".mcmc.means")){
    ...
    //std::cerr << mcmc_loglike(settings, mcmc_mat, data)  << '\n';
    // } else if(check_file_exists(settings.outbase+".mcmc")&& false){
