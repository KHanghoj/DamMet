...
void add_aligned_data(const general_settings &settings,
  ...
  for (size_t i = 0; i < d.t_seq.size()-1; i++){
    ...
    if(include_in_cpg){
      ...
      // if(base_composition==2){
      // 	std::cerr << p_per_site->position << " " << r_base1<<r_base2 << " " << s_base1<<s_base2 <<'\n';
      // }
...
int parse_bam(int argc, char * argv[]) {
  ...
  if (!settings.exclude_for_tally_fn.empty()) {
    std::cerr << "\t-> Loading file to mask het and hom alt sites" << '\n';
    filter_ref_db(settings.chrom, settings.exclude_for_tally_fn, ref);
  }
  ...
  while (sam_itr_next(in, iter, rd) >= 0) {
   ...
   if (rd->core.l_qseq < settings.minreadlength || rd->core.qual < settings.minmapQ || (rd->core.flag & settings.flags_off)){
      trashed++;
      continue;
    }
   ...
   // when adding the rd to data, we should also add the RG as a vector. It could be integers referecing the all the RG found in the header.. 
    ...
    d = align_read(rd, ref);
    add_aligned_data(settings, d, cpg_map, ref, data, tm, nocpg_data);
  }
  ...
  std::cerr << "\t-> Total_Observations CpG: " << cpgsdepth  << ". CnoCpG: " << nocpg_data.depth << '\n';
  ...
  if(!settings.deamrates_filename.empty() && check_file_exists(settings.deamrates_filename)){
    ...
    param_deam = load_deamrates_f(settings);
  } else if (check_file_exists(settings.outbase+".deamrates")){
