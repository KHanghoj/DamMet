...
double objective_func(const std::vector<double> &x, std::vector<double> &grad, void *my_func_data)
{
  ...
  for (const auto &site : (*d->data)) {
    for (size_t i=0; i < site.depth; i++){
      ...
      if (site.base_compos[i] == 0) {
        ...
        if (!grad.empty()) {
          grad[idx_param_meth] += d->settings.M * (1-maperror) * (seqerror * 4.0 / 3.0 - 1);
          grad[idx_param_unmeth] += (1-maperror) * (-d->settings.M + 1) * (seqerror*4.0/3.0 - 1);
        }
        ...
        if (!grad.empty()) {
          grad[idx_param_unmeth] += ((1-maperror) * (-d->settings.M + 1) * (- seqerror*4.0/3.0 + 1));
          grad[idx_param_meth] += (1-maperror) * d->settings.M * (-seqerror * 4.0 / 3.0 + 1);
        }
  ...
  for(size_t i=0; i< d->nocpg_data->depth; i++){
    ...
    if (d->nocpg_data->base_compos[i] == 0) {
      ...
      if (!grad.empty()) {
        grad[idx_param_unmeth] += (1-maperror) * (seqerror * 4.0 / 3.0 - 1);
      }
...
void run_mcmc(const general_settings & settings, const tally_mat_freq & tmf, std::vector<per_site> & data, per_site_nocpg & nocpg_data ){
  ...
  std::vector<double> t;
  std::cout << "Initial (param are freq from tallycounts file) llh: " << objective_func(param, t, &void_stuff) << std::endl;
  ...
  std::cout << "Optim return code: " << result << std::endl;
  std::cout << "iterations: " << void_stuff.iteration << std::endl;
