...
double objective_func_deamrates(const std::vector<double> &x, std::vector<double> &grad, void *my_func_data)
{
  ...
  for(size_t i=0; i< d->nocpg_data->depth; i++){
    if(d->rg_idx != d->nocpg_data->rgs[i]){
      continue;
    }
...
void run_deamrates_optim(const general_settings & settings, const tally_mat_freq & tmf, std::vector<per_site> & data, per_site_nocpg & nocpg_data , std::string & rg, size_t & rg_idx){
...
int parse_bam(int argc, char * argv[]) {
  ...
  for (size_t i=0; i<rgs.size(); i++){
    std::cerr << "\t-> Total_Observations RG: "<< rgs[i] << " CpG: " << cov_rg.cpg[i]  << ". CnoCpG: " << cov_rg.nocpg[i] << '\n';
  }
  for (size_t i=0; i<rgs.size(); i++){
  ...
  for (size_t i=0; i<rgs.size(); i++){
    ...
    if(!settings.deamrates_filename.empty() && check_file_exists(settings.deamrates_filename)){
      ...
      run_deamrates_optim(settings, tmf[i], data, nocpg_data, rgs[i], i);
