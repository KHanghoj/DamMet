...
void dump_count_file(general_settings & settings, tally_mat & tm, const std::string & rg){
...
tally_mat_freq read_count_file(general_settings & settings, const std::string & rg){
...
void run_deamrates_optim(const general_settings & settings, const tally_mat_freq & tmf, std::vector<per_site> & data, per_site_nocpg & nocpg_data , const std::string & rg, size_t rg_idx, my_cov_rg & cov_rg){
...
std::vector<double> load_deamrates(const general_settings & settings, const std::string & rg){
...
int parse_bam(int argc, char * argv[]) {
  ...
  // load read groups
  bool rg_split;
  std::unordered_map<std::string,size_t> rgs;
  ...
  for (const auto & val: rgs){
    tm[val.second] = init_tallymat(settings.max_pos_to_end);
  }
  ...
  while (sam_itr_next(in, iter, rd) >= 0) {
   ...
   if(rg_split){
     ...
     if(rgptr==NULL){
       ...
       auto match = rgs.find(rgname);
       if(match != rgs.end()){
	 rgname_idx = match->second;
   ...
   d = align_read(rd, ref);
   add_aligned_data(settings, d, cpg_map, ref, data, tm[rgname_idx], nocpg_data, cov_rg, rgname_idx);
  }
  ...
  for (const auto & val: rgs){
    std::cerr << "\t-> Total_Observations RG: "<< val.first << " CpG: " << cov_rg.cpg[val.second] << " CpGCoverage: " << (double)cov_rg.cpg[val.second]/(double)cpg_map.size() <<  ". CnoCpG: " << cov_rg.nocpg[val.second] << '\n';
  }
  for (const auto & val: rgs){
    std::cerr << "\t-> Dumping count file: " << settings.outbase << "." << val.first <<  ".tallycounts" << '\n';
    dump_count_file(settings, tm[val.second], val.first);
    tmf[val.second] = read_count_file(settings, val.first);
  }
  ...
  for (auto & val: rgs){
    ...
    // filter_per_site_data(settings, tmf[i], data[i], val.first);
    ...
    if(!settings.deamrates_filename.empty() && check_file_exists(settings.deamrates_filename)){
      ...
      param_deam[val.second] = load_deamrates_f(settings);
    } else if (check_file_exists(settings.outbase+"."+val.first+".deamrates")){
      std::cerr << "\t-> Loading deamination rates from " << settings.outbase+"."+val.first+".deamrates" << '\n';
      param_deam[val.second] = load_deamrates(settings, val.first);
    }else {
      std::cerr << "\t-> Starting Optim of deamination rates." << '\n';
      run_deamrates_optim(settings, tmf[val.second], data, nocpg_data, val.first, val.second, cov_rg);
      std::cerr << "\t-> Dumping deamination parameters to " << settings.outbase+"."+val.first+".deamrates" << '\n';
      param_deam[val.second] = load_deamrates(settings, val.first);
    }
  }
