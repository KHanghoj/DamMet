  ...
  for (size_t i = 0; i < d.t_seq.size()-1; i++){
	...
	// continue;
      ...
      if(no_ref_cpg(r_base1, r_base2)){
        if(refToInt[(int)ref_tally[d.t_positions[b2]]]==2 &&
...
int parse_bam(int argc, char * argv[]) {
  ...
  // std::set<size_t> cpg_set;
  // for (const auto & val: cpg_map){
  //   cpg_set.emplace(val.first);
  // }
  ...
  while (sam_itr_next(in, iter, rd) >= 0) {
   ...
   if (rd->core.l_qseq < settings.minreadlength || rd->core.qual < settings.minmapQ || (rd->core.flag & settings.flags_off)){
    ...
    d = align_read(rd, ref);
    add_aligned_data(settings, d, cpg_map, ref_tally, data, tm, nocpg_data);
  }
