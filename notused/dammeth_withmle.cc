...
ll_data_d_run mcmc_loglike_incl_nocpg(const general_settings &settings,
                                    const mcmc_meth_mat &d,
                                    const std::vector<per_site> &data,
                                    const per_site_nocpg &nocpg_data) {
  ll_data_d_run to_return;
  mcmc_meth_mat slope = init_zero_mcmc_meth_mat(settings);
  long double ll = 0;
  double res=0;
  double seqerror, deamin_methylated, deamin_unmethylated;
  double maperror;
  double noM, M;
  double * noM_slope,  * M_slope;
  for (const auto &site : data) {
    for (size_t i=0; i < site.depth; i++){
      ...
      M_slope = &slope[0][site.pos_to_end[i]][site.prime[i]];
      noM_slope = &slope[1][site.pos_to_end[i]][site.prime[i]];
      ...
      if (site.base_compos[i] == 0) {
        ...
        // see diff help in .org file
        (*noM_slope) += ((1-maperror) * (-settings.M + 1) * (seqerror*4.0/3.0 - 1)) / res;
        (*M_slope) +=   ((1-maperror) * settings.M * (seqerror * 4.0 / 3.0 - 1)) / res;
  ...
  for(size_t i=0; i<nocpg_data.depth; i++){
    ...
    noM_slope = &slope[1][nocpg_data.pos_to_end[i]][nocpg_data.prime[i]];
    if (nocpg_data.base_compos[i] == 0) {
      // C->C -> (1-e) * (1-d) + (d * e/3)
      noM = (1 - seqerror) * (1 - deamin_unmethylated) + (deamin_unmethylated * seqerror / 3.0);
      res = (1-maperror) * noM  + maperror * BASE_FREQ_FLAT_PRIOR;
      (*noM_slope) += (1-maperror) * (seqerror * 4.0 / 3.0 - 1) / res;
  ...
  to_return.ll = ll;
  to_return.slope = slope;
  return to_return;
}

void update_deam_param(const general_settings & settings, mcmc_meth_mat & d_old, mcmc_meth_mat & d_new, mcmc_meth_mat & slopes, double & ALPHA){
  // double BETA = 0.5;  // this is BETA * z + ll_run.slope, where z == the prevoius slope
  double tole = 1e-10;
  for (size_t i=0; i<2;i++){
    for (size_t p=0; p<settings.max_pos_to_end+2;p++){
      for (size_t pr=0; pr<PRIMES;pr++){
        ...
        d_new[i][p][pr] = d_old[i][p][pr] + ALPHA * slopes[i][p][pr];
...
void run_mcmc(const general_settings & settings, const tally_mat_freq & tmf, std::vector<per_site> data, const per_site_nocpg & nocpg_data ){
  double ALPHA = 1e-11;
  double tole = 1e-15;
  mcmc_meth_mat d_old = init_mcmc_meth_mat(settings, tmf); // initial guess
  mcmc_meth_mat d_temp = init_mcmc_meth_mat(settings, tmf);
  ll_data_d_run ll_runold, ll_run;
  ll_runold = mcmc_loglike_incl_nocpg(settings, d_old, data, nocpg_data);
  ...
  size_t c_iter = 0;
  fprintf(stderr, "startlik: %Lf\n", ll_runold.ll);
  update_deam_param(settings, d_old, d_temp, ll_runold.slope, ALPHA);
  d_old = d_temp;
  for (c_iter = 0; c_iter < niter; c_iter++) {
    ll_run = mcmc_loglike_incl_nocpg(settings, d_old, data, nocpg_data);
    update_deam_param(settings, d_old, d_temp, ll_run.slope, ALPHA);

    d_old = d_temp;

    if (c_iter % 10 == 0 && c_iter) {
      fprintf(stderr, "[%lu] lik=%Lf diff=%Le\n", c_iter, ll_run.ll,
              fabs(ll_run.ll - ll_runold.ll));
    }
  ...
  print_mcmc_meth_mat(settings, f, d_old, 1, ll_runold.ll);
}
...
void mle_loglike(ll_data_f_run & d, const double & f, const std::vector<pre_calc_per_site> & data, const per_mle_run & mle_data){

  double res=0, ll=0, slope=0, second_der=0;
  double noM, M;
  double mape;
  for (const auto idx : mle_data.idx_to_include){
    for (size_t i=0; i < data[idx].depth; i++){
      ...
      res = (1-mape) * ((1-f) * noM + f * M) + mape*BASE_FREQ_FLAT_PRIOR;
      ll += std::log(res);
...
void run_mle(const general_settings & settings,
  ...
  for (size_t curr_idx = 0; curr_idx < pre_calc_data.size(); curr_idx++) {
    ...
    size_t niter = 20000000;
    size_t c_iter=0;
    ll_data_f_run ll_run, ll_runold;
